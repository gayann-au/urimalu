...
// ============================================
// Web push (price alerts, stage 3)
// The send-push Edge Function posts a JSON payload { title, body, url }. The
// text is already composed server side from the notification's structured
// fields, so the worker just displays it. url defaults to the notifications
// page so a tap lands there.
// ============================================
self.addEventListener("push", (event) => {
  ...
  const title = payload.title || "Urimalu";
  const options = {
    body: payload.body || "",
...
// Tapping a push focuses an open Urimalu tab and routes it to the target page,
// or opens a new one if none is open.
